...
# deleting comments checking if user is this comment's author
...
class BlogPost(db.Model):
    ...
    # ***************Setting up Parent Relationship for comments*************#
...
def calculate_time_difference(posted_time):
    ...
    # the divmod divide the time_difference.seconds by 3600 and then save the answer as a tuple, with the whole
    # number and remainder, as hours and remainder(minutes) respectively. the same as the second tuple, only that the
    # remainder( _ ) which is the seconds is not needed.
...
def register():
    ...
    if form.validate_on_submit():
        ...
        user = result.scalar()
        if user:
            flash("You are already registered, log in instead!", 'success')
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            flash("That email doesn't exist in our directory!")
            return redirect(url_for('login'))
